...
/*
Fighter Generator - Labyrinth Lord
...
int main(void)
{
	/*Holds the fighter's name*/
	char fighterName[ARRAY_SIZE];

	/*Holds the fighter's alignment*/
	...
	/*Saving throws*/
	unsigned breathAttacks = 15;
	unsigned poisonDeath = 12;
	...
	/*Introduction text*/
	printf_s("Welcome to the Labyrinth Lord Fighter Generators.\n", ARRAY_SIZE);

	/*prompt user to enter fighter's name*/
	printf_s("\nEnter the name of your fighter: \n", ARRAY_SIZE);

	scanf_s("%[^\n]s", fighterName, ARRAY_SIZE);
	...
	printf_s("\nYou have %u gold pieces (gp) to spend on your fighter.  Now it is time to buy some armour, weapons and gear.\n\n", gold, ARRAY_SIZE);
	...
	/*Option of purchasing a shield if fighter has at least 10 gp*/
	if (gold >= 10)
	{
	...
	printf_s("\nNow, let's purchase some weapons for your fighter.\n", ARRAY_SIZE);

	/*Function for adding weapons for fighter*/
	do
	{
		choice = 0;
		...
		printf_s("3. Battle Axe       6 gp      1d8\n", ARRAY_SIZE);
		...
		case 3:
			if (gold > 6)
			{
				expense = 6;
				gold -= expense;
				strcat(weapons, "Battle Axe");
				printf_s("\nYou have purchased a Battle Axe\n", ARRAY_SIZE);
			}
			else
			{
				printf_s("\nYou do not have enough gold for the Battle Axe\n", ARRAY_SIZE);
			}
	...
	/*Function for adding gear for fighter*/
	do
	{
		choice = 0;
	...
	printf_s("\nHere is an update of your Fighter:\n", ARRAY_SIZE);
	...
	printf_s("\nDo you want to save your Fighter (%s) to a text files?\n1.yes\n2.no\n(please select 1 (yes) or 2 (no)\n", fighterName, ARRAY_SIZE);

	scanf(" %d", &noYes);

	if (noYes == 1)
	{
		printf_s("\nPlease a name (no spaces, please) for the file you want to save the Fighter's information to: ", ARRAY_SIZE);
...
/*Randomly generates the Fighter's hit points
...
/*displays the prime requisite bonus for fighter*/
char* primeReqBonus(unsigned strength)
{
